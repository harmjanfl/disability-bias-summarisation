...
def main():
    ...
    # --- pairwise summary-level Welch two-sample t-tests ---
    # Each generated summary is one observation; the two architectures are
    # compared with a Welch t-test on their per-summary visibility proportions.
    results = []
    ...
    for c in conditions:
        for a, b in combinations(model_names, 2):
            ...
            if c not in ma.index or c not in mb.index:
                continue
            # summary-level mean estimates (for reference in output)
            p1 = ma.at[c, "V_hat"]
