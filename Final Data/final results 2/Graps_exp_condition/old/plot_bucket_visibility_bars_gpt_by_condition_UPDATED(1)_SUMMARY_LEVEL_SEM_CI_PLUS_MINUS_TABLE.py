...
and 95% Wilson confidence intervals. Pairwise z-tests are saved as CSV.
...
def topic_level_counts(df: pd.DataFrame, model: str, condition, bucket: str):
    """Return total successes and trials (visible topics, total topics) for model/condition/bucket."""
    sub = df[(df["present_in_input"] == 1) & (df["model"] == model) & (df["experiment_condition"] == condition) & (df["bucket"] == bucket)].copy()
    if sub.empty:
        return 0, 0
    per_iter = (
        sub.groupby(["iteration"]).agg(visible_topics=("V_t", "sum"), total_topics=("V_t", "size")).reset_index()
    )
    ...
    for c in conditions:
        for a, b in combinations(model_names, 2):
            ...
            results.append({
                ...
                "p_hat_a": p1, "n_a_summary": n1_summary, "p_hat_b": p2, "n_b_summary": n2_summary,
