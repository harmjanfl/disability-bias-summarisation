...
of input), and 95% CIs based on per-iteration variability (SEM-based).
Pairwise two-proportion z-tests are computed at the topic level (pooled
counts across iterations) and saved as CSV/XLSX.

Reads:  Final Data/final results 2/combined/combined_iteration_topic.xlsx
Writes: Final Data/final results 2/combined/graphs by condition/plot_visibility_dis_cluster_bars_by_condition.png
        Final Data/final results 2/combined/graphs by condition/plot_visibility_dis_cluster_stats_by_condition.csv
        Final Data/final results 2/combined/graphs by condition/plot_visibility_dis_cluster_stats_by_condition.xlsx
        Final Data/final results 2/combined/graphs by condition/plot_visibility_dis_cluster_table.xlsx
"""
...
OUTPUT_STATS_XL = "Final Data/final results 2/combined/graphs by condition/plot_visibility_dis_cluster_stats_by_condition.xlsx"
...
def two_prop_z_test(p1, n1, p2, n2):
    """Pooled two-sided two-proportion z-test."""
...
def topic_level_counts(df: pd.DataFrame, model: str, condition, bucket: str):
    """Return (successes, trials) at the topic level for a (model, condition, bucket) cell.

    successes = total number of (iteration x topic) pairs where the topic
                was present in the input AND survived into the summary.
    trials    = total number of (iteration x topic) pairs where the topic
    ...
    per_iter = (
        ...
        .agg(visible_topics=("V_t", "sum"),
             total_topics=("V_t", "size"))
        .reset_index()
    )
    x = int(per_iter["visible_topics"].sum())
    N = int(per_iter["total_topics"].sum())
    return x, N
...
def aggregate_by_condition(df: pd.DataFrame, bucket: str) -> pd.DataFrame:
    ...
    This produces one observation per generated summary, treating the
    iteration (not the topic) as the independent unit of analysis.
    """
    sub = df[
...
def main():
    ...
    # --- pairwise topic-level two-proportion z-tests ---
    results = []
    ...
    for c in conditions:
        for a, b in combinations(model_names, 2):
            ...
            if c not in ma.index or c not in mb.index:
                continue
            # summary-level estimates (for reference in output)
            p1 = ma.at[c, "V_hat"]
            n1_summary = int(ma.at[c, "n"]) if not pd.isna(ma.at[c, "n"]) else 0
            p2 = mb.at[c, "V_hat"]
            n2_summary = int(mb.at[c, "n"]) if not pd.isna(mb.at[c, "n"]) else 0

            # topic-level counts (used for the z-test itself)
            x1, N1 = topic_level_counts(df, a, c, BUCKET)
            x2, N2 = topic_level_counts(df, b, c, BUCKET)

            if N1 == 0 or N2 == 0:
                z, pval = math.nan, math.nan
                sig = False
            else:
                pa = x1 / N1
                pb = x2 / N2
                z, pval = two_prop_z_test(pa, N1, pb, N2)
