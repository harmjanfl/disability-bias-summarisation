...
def topic_labeler_node(state: AgentState) -> AgentState:
    ...
    raw_output = invoke_local_llm(messages)
...
def summarizer_node(state: AgentState) -> AgentState:
    ...
    raw_output = invoke_local_llm(messages, max_new_tokens=DEFAULT_MAX_NEW_TOKENS)
    ...
    if mode_mismatch:
        errors.append(f"Summarizer mode mismatch: expected {task_mode}, got {echoed_mode}.")
    if hash_mismatch:
        errors.append(
    ...
    return {
        ...
        "summary_sentences_structured": revised_sentences,
        "errors": errors,
        "metadata": metadata,
    }
